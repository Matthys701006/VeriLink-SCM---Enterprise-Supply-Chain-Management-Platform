/*
  ...
  This migration adds analytics snapshot functionality and ML model tracking to the
  VeriLink SCM platform, providing AI-powered insights and predictive capabilities.

  1. New Tables
    - `analytics_snapshots` - Stores periodic analytics snapshots
    - `ml_models` - Tracks ML model metadata and versioning
...
-- Add RLS policies (with DROP IF EXISTS to avoid conflicts)
DROP POLICY IF EXISTS "Users can view organization analytics" ON analytics_snapshots;
CREATE POLICY "Users can view organization analytics"
  ON analytics_snapshots FOR SELECT
  TO authenticated
  USING (organization_id = (SELECT organization_id FROM users WHERE id = auth.uid()));

DROP POLICY IF EXISTS "Users can manage organization ML models" ON ml_models;
CREATE POLICY "Users can manage organization ML models"
  ON ml_models FOR ALL
  TO authenticated
  USING (organization_id = (SELECT organization_id FROM users WHERE id = auth.uid()));

-- First make sure the default organization exists before inserting sample data
DO $$
DECLARE
    org_exists boolean;
BEGIN
    ...
    IF NOT org_exists THEN
        ...
        INSERT INTO organizations (
            ...
            'DEFAULT',
            'Default organization for VeriLink SCM system',
...
-- Insert sample analytics data (only after ensuring organization exists)
...
-- Insert sample ML models (only if organization exists)
