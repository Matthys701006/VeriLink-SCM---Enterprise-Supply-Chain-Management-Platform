/*
# Fix Compliance Records Schema and Sample Data

1. New Tables
  - `compliance_records` - Table to track regulatory compliance records

2. Security
  - Enable RLS on `compliance_records` table
  - Add policy for authenticated users to view their organization's compliance records
...
-- Sample Compliance Records - Fixed entity_id UUID generation
...
SELECT
  ...
  '[]',
  CASE 
    WHEN (i % 3) = 1 THEN '[{"violation": "Missing documentation", "severity": "medium"}]'
    ELSE '[]'
  END,
