...
export default function Compliance() {
  ...
  } = useSupabaseData<ComplianceRecord>(
    "compliance_records",
  ...
  // Process data when Supabase response changes
  useEffect(() => {
    if (!loading) {
      if ((supabaseRecords?.length === 0 || !supabaseRecords) || error) {
        // Use mock data if no records or error
        const mockData = generateMockData();
        setComplianceData(mockData);
        setUsesMockData(true);
        
        if (error) {
          console.warn('Using mock compliance data due to error:', error);
        } else {
          console.info('No compliance records found, using mock data');
        }
      } else {
        // Use actual data
        setComplianceData(supabaseRecords);
        setUsesMockData(false);
      }
    }
  }, [supabaseRecords, loading, error]);

  // Refresh function that handles both real and mock data
  const refreshData = () => {
    if (usesMockData) {
      // If using mock data, just refresh the mock data
      setComplianceData(generateMockData());
      toast({
        title: "Data Refreshed",
        description: "Using sample compliance data for demonstration"
      });
    } else {
      // Otherwise use the actual refetch
      supabaseRefetch();
      toast({
        title: "Data Refreshed",
        description: "Latest compliance data has been loaded"
      });
    }
  };
  ...
  const compliantRecords = complianceData.filter(r => r.compliance_status === 'compliant').length || 0
  const pendingRecords = complianceData.filter(r => r.compliance_status === 'pending').length || 0
  const nonCompliantRecords = complianceData.filter(r => r.compliance_status === 'non_compliant').length || 0
  ...
  const upcomingReviews = complianceData.filter(record => {
  ...
  const expiringCertifications = complianceData.filter(record => {
  ...
  const regulationTypes = [...new Set(complianceData.map(r => r.regulation_type) || [])]
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Monitor regulatory compliance, certifications, and audit requirements
              {usesMockData && (
                <span className="ml-1 text-yellow-600 font-medium text-sm">(using sample data)</span>
              )}
            </p>
          </div>
